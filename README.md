...
Изначально (апрель 2025, `v0.1`) — прямолинейный прототип на официальной на
тот момент библиотеке `tinkoff-investments` (`tinkoff.invest`), с
дублированным кодом на двух хостах. В июле 2026, после того как
`tinkoff-investments` ушла в карантин на PyPI (наследие переименования
Тинькофф → Т-Банк — сама библиотека переименована не сразу, только со
временем появился официальный преемник), код реструктурирован в текущий вид
и перенесён на официальную библиотеку Т-Банка `t-tech-investments`
(`t_tech.invest`).
